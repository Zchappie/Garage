...
class Fabric:
    ...
    def parse_input(self, input_file):
        with open(input_file) as f:
            ...
            blocks = set([tuple(map(tuple,block)) for block in blocks])
            blocks = list(sorted(blocks)) # sort again for deterministic output
            blocks = [list(map(list,block)) for block in blocks]
